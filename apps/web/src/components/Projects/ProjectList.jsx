...
import { Building2, Plus, MapPin, Calendar, ShieldCheck, Edit3, Trash2, ChevronRight, AlertTriangle, CheckCircle2, AlertOctagon, ShieldAlert, Search } from 'lucide-react';
...
export default function ProjectList({ onSelectProject }) {
  ...
  return (
    <div className="space-y-6">
      ...
      {isNcaRegulator && (
        <div data-testid="nca-portal-banner" className="bg-amber-500/10 border border-amber-500/20 rounded-xl p-4 flex items-center justify-between text-xs text-amber-300">
          <div className="flex items-center space-x-3">
            <ShieldAlert className="w-5 h-5 text-amber-400 shrink-0" />
            <div>
              <span className="font-bold">National Construction Authority (NCA) Regulatory Audit Portal — Read-Only Mode</span>
              <p className="text-[11px] text-slate-400 mt-0.5">
                You possess nationwide compliance inspection privileges. All project mutations and deletions are restricted.
              </p>
            </div>
          </div>
          <span className="px-2.5 py-1 rounded bg-amber-500/20 text-amber-300 font-semibold text-[10px] uppercase tracking-wider shrink-0">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-slate-900 border border-slate-800 rounded-xl p-5">
        <div>
          <h2 className="text-xl font-bold text-white tracking-tight flex items-center space-x-2">
            ...
            <span>{isNcaRegulator ? 'Nationwide Construction Audit Registry' : 'Active Construction Projects'}</span>
          </h2>
      ...
      <div className="flex flex-col sm:flex-row gap-3 bg-slate-900/60 p-3 rounded-xl border border-slate-800">
        ...
        <div className="flex items-center space-x-2">
          <label className="text-xs text-slate-400 whitespace-nowrap">NCA Grade:</label>
          <select
            value={gradeFilter}
            onChange={(e) => setGradeFilter(e.target.value)}
            className="bg-slate-950 border border-slate-800 rounded-lg px-2.5 py-1.5 text-xs text-white focus:outline-none focus:border-sky-500"
          >
